...
module.exports = function(args, flags) {
  ...
  let isSingle = args.torrent.kind === args.constants.kind.single
  def = {
    unsorted: 'n',
    music: flags.isMusic ? 'y': 'n',
    artwork: config.fetchArtworks ? 'y' : 'n',
    ut_title: args.torrent.name,
    ut_dir: isSingle ? args.torrent.directory : normalize(args.torrent.directory) + '/' + args.torrent.name, 
  ...
  console.log('Executing "filebot ' + cmd + '"');
  if (!config.dryRun) {
      var output;
      try {
